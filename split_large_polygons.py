...
def main():
    ...
    try:
        ...
        while True:
            ...
            step += 1
            cur.execute(fmt("select count(*) as count from {table} where ST_Area(ST_Transform({column},4326)::geography) > {area};"))
            row = cur.fetchall()
            ...
            sql = fmt("select {id} as id, st_xmin({column}) as xmin, st_ymin({column}) as ymin, st_xmax({column}) as xmax, st_ymax({column}) as ymax from {table} where ST_Area(ST_Transform({column},4326)::geography) > {area} order by ST_Area(ST_Transform({column},4326)::geography) DESC")
            cur.execute(sql)
            rows = cur.fetchall()
